...
func main() {
	...
	for index := range sequences {
		...
		nextValue := getNextValue(sequence, coefficients)
		sumOfNextValues = sumOfNextValues + nextValue
	}

	fmt.Println(sumOfNextValues)
}

func getNextValue(sequence Sequence, coefficients [][]int) int {
	coefficientsForSequence := coefficients[sequence.length + 1]
	nextValue := 0
	for index := range sequence.values {
		value := sequence.values[index]
		coefficient := coefficientsForSequence[index]
